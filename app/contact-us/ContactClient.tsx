    ...
    if (!isDesktop) {
        return (
            <div className="bg-[#0e1626] min-h-screen font-sans text-white pt-24 pb-16">
                ...
                {/* 2. Full-width map — pin clickable, popup centered on map */}
                <motion.div
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                ...
                >
                    ...
                    {/* Popup centered on the map — always fits on screen */}
                    <AnimatePresence>
                        {showInfo && (
                            <motion.div
    ...
    return (
        <div className="min-h-screen relative overflow-hidden font-sans text-white pt-28 md:pt-32 pb-12 px-4 md:px-8 lg:px-16 bg-[#0e1626]">
            ...
            <div className="absolute inset-0 z-0 lg:z-auto">
                <StyledMapBackground
                    onPinReady={() => {}}
                    onPinClick={(clientX, clientY) => {
                        const popupWidth = 440;
                        const popupHeight = 340;
                        const x = Math.min(clientX + 60, window.innerWidth - popupWidth - 16);
                        const y = Math.min(Math.max(clientY - popupHeight / 2, 120), window.innerHeight - popupHeight - 16);
                        setPopupPos({ x, y });
                        setShowInfo(v => !v);
                    }}
                />
